  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-white tracking-tight">Inventory Movement Audit Log</h2>
          <p className="text-xs text-slate-400">Immutable audit trail of all warehouse stock inflows (IN) and sales outflows (OUT)</p>
        </div>
            ...
            <tbody className="divide-y divide-slate-800/50">
              {loading ? (
                ...
                movements.map((move) => {
                  ...
                  return (
                    <tr key={move.id} className="hover:bg-slate-800/30 transition">
                      ...
                      <td className="py-3.5 px-4">
                        <p className="font-semibold text-white">{move.product?.name || 'Product'}</p>
                        <p className="text-[11px] text-slate-400 font-mono">SKU: {move.product?.sku}</p>
                      </td>
                      <td
                      ...
                      <td className="py-3.5 px-4 text-slate-300 max-w-xs truncate">{move.reason}</td>
                      <td className="py-3.5 px-4 text-right text-slate-400">
                        <p className="font-medium text-slate-200">{move.author?.fullName || 'System'}</p>
                        <p className="text-[10px] text-sky-400 uppercase font-semibold">{move.author?.role}</p>
                      </td>
                    </tr>
                  );
                })
